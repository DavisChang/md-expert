...
Markdown Expert 會把這些內容轉成一致的閱讀體驗，包含標題、表格、任務清單、程式碼區塊、Mermaid 圖表、LaTeX 數學式、語法高亮與目錄導覽。
...
- **豐富 Markdown 渲染**：支援表格、任務清單、fenced code、Mermaid 圖表與 LaTeX 數學式。
...
| 語法 | 範例 |
| --- | --- |
| Mermaid 圖表 | ```` ```mermaid ```` fenced blocks |
| 行內數學式 | `$x^2 + y^2 = z^2$` |
| 區塊數學式 | `$$ E = mc^2 $$` |
